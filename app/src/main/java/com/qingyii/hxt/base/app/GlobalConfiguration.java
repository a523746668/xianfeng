...
import com.qingyii.hxt.base.mvp.api.Api;
                    ...
                    public Request onHttpRequestBefore(Interceptor.Chain chain, Request request) {
                        /* 如果需要再请求服务器之前做一些操作,则重新返回一个做过操作的的requeat如增加header,不做操作则直接返回request参数
                           return chain.request().newBuilder().header("token", tokenId)
                                  .build(); */
                        Request new_request = chain.request().newBuilder().addHeader("Accept:", XrjHttpClient.ACCEPT_V2).addHeader("Authorization", MyApplication.hxt_setting_config.getString("credentials", ""))
                                .build();

//                        if(new_request.url().toString().startsWith(XrjHttpClient.URL_UP)) {
//                            ProgressManager.getInstance().addResponseListener(new_request.url().toString(), WindowUtils.getDownloadListener());
//                        }
                ...
                .responseErrorListener((context1, t) -> {
                    ...
                    Log.i("tmdglobal",t.getMessage().toString());
                ...
                .okhttpConfiguration((context1, okhttpBuilder) -> {//这里可以自己自定义配置Okhttp的参数
                    okhttpBuilder.writeTimeout(10, TimeUnit.SECONDS).connectTimeout(10,TimeUnit.SECONDS).readTimeout(10,TimeUnit.SECONDS);
...
//                    });
               ...
               NeverCrash.init((t, e) -> {
                   ...
                   /* Log.i("tmdcrash1" , Build.VERSION.RELEASE+"-----"+Build.VERSION.SDK_INT+"------"+Build.MANUFACTURER+"----"
                            +Build.MODEL);
                    //Log.i("tmdcrash2",e.getMessage());
                    Log.i("tmdcrash2",e.toString());

                    StackTraceElement[] st=e.getStackTrace();
                    for(StackTraceElement se:st){
                        Log.i("tmdcrash3",se.toString());
                        Log.i("tmdcrash4",se.getClassName()+"----"+se.getFileName()+"-----"+se.getMethodName()+"----"+se.getLineNumber());
                    }*/
                    e.printStackTrace();
                });
//                //leakCanary内存泄露检查
