...
import com.zhf.Util.PictureCompressUtil;
import com.zhf.base.Basepresenter;
import com.zhf.bean.Fabubean;
import com.zhf.bean.SendTask;
import com.zhf.bean.TaskTag;
import com.zhf.bean.UploadVideobean;
import com.zhf.bean.Uploadimagebean;
...
public class IssPresenter extends Basepresenter<Isstaskview> {
    ...
    //保存草稿箱
     ...
     public void gettags(){
         String murl="http://web.seeo.cn/check/getTag?token="+ MyApplication.hxt_setting_config.getString("token","");
         OkHttpUtils.get()
                 .url(murl)
                 .build()
    ...
    private void singleUploadPic(List<File> parts, File part,String id) {
       ...
       File savepath=compressbitmap(part);
        OkHttpUtils.post()
                ...
                .addFile("picture",getFileName(savepath.getPath()),savepath)
                .build()
    ...
    private void singleUploadPic1(List<File> parts, File part,String id) {
       ...
       //上传图片之前进行压缩
        ...
        OkHttpUtils.post()
                ...
                .addHeader("Authorization", MyApplication.hxt_setting_config.getString("credentials", ""))
                .addParams("user_id", MyApplication.userUtil.getId() + "")
                .addFile("picture",getFileName(save.getPath()),save)
                .build()
                .execute(new uploadpicback() {
                    @Override
                    public void onError(Call call, Exception e, int id) {
  ...
  //发布
    ...
    , ArrayList<String> files, String score_text, int is_show, int is_show_auditing, int show_range, int is_comment, String situation,
                     String address){
      String murl= Urlutil.baseurl+"/check/activityAct/"+Global.taskdetailbean.getData().getTask().getId()+"?token="+MyApplication.hxt_setting_config.getString("token","");
       ...
       Log.i("tmdfabu",tasks);
        OkHttpUtils.post()
                ...
                .execute(new Fabuback() {
                    @Override
                    public void onError(Call call, Exception e, int id) {
                        Log.i("tmdfabu",e.getMessage().toString());
                        HintUtil.stopdialog();
                    }
   ...
   //上传视频
      public void uploadvideo(SendTask task){
       HintUtil.startupload(context);
        String murl="http://admin.seeo.cn/api/upload/upvideo";
           OkHttpUtils.post()
                   .url(murl)
                   .addHeader("Accept", XrjHttpClient.ACCEPT_V2)
                   .addHeader("Authorization", MyApplication.hxt_setting_config.getString("credentials", ""))
                   .addParams("user_id", MyApplication.userUtil.getId() + "")
                   .addFile("uploadvideo",getFileName(task.getUri()),new File(task.getUri()))
                   .build()
                   .execute(new UploadVideoBack() {
                       ...
                       public void onResponse(UploadVideobean response, int id) {
                           ...
                           if(response.getError_code()==1){
                                 task.setWeburl(response.getData());
                                 Log.i("tmdupload",task.getWeburl());
                                 Acvitivtyview.uploadvideosuccess(task);
                             } else {
                                 Toasty.error(context,"视频上传失败："+response.getData(),1).show();
                             }
                       }
                   });
