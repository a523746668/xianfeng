...
public final class FBReader extends FBReaderMainActivity implements ZLApplicationWindow {
	...
	public List<TOCTree> getTocList() {
		if (tocList == null) {
			tocList = new ArrayList<TOCTree>();
	...
	private void getTocList(TOCTree tocTree) {
		for (TOCTree tocTree1 : tocTree.subtrees()) {
			tocList.add(tocTree1);
			if (tocTree1.subtrees().size() != 0) {
				getTocList(tocTree1);
			}
		}
	}
	public void turnChapter(boolean isNext) {
		getTocList();
		TOCTree curTocTree = myFBReaderApp.getCurrentTOCElement();
		// 当封面不属于TocTree的时候，当个别书籍有目录，但是Toctree.getRefrence.ParagraphIndex得到的值都一直是，
		// 返回的TocTree也是null，这个时候，会崩溃，所以做特殊处理
		if (curTocTree == null) {
			return;
		}
		String text = curTocTree.getText();
		for (int i = 0; i < tocList.size(); i++) {
			if (tocList.get(i).getText().equals(text)) {
				if (isNext) {
					if (i == tocList.size() - 1) {
						Toast.makeText(this, "到达最后一章", Toast.LENGTH_SHORT).show();
					} else {
						myFBReaderApp.getTextView().gotoPosition(tocList.get(i + 1).getReference().ParagraphIndex, 0, 0);
						getViewWidget().repaint();
					}
				} else {
					if (i == 0) {
						Toast.makeText(this, "到达第一章", Toast.LENGTH_SHORT).show();
					} else {
						myFBReaderApp.getTextView().gotoPosition(tocList.get(i - 1).getReference().ParagraphIndex, 0, 0);
						getViewWidget().repaint();
					}
	...
	protected void onPause() {
		...
		msg.what = EventBusTags.UPDATE_BOOK_READ_PROGRESS;
		msg.arg1 = myFBReaderApp.getTextView().pagePosition().Current;
		msg.obj = myFBReaderApp.getTextView().pagePositionPec();
		EventBus.getDefault().post(msg,EventBusTags.BOOK_PROGRESS);
		IsPaused = true;
		try {
