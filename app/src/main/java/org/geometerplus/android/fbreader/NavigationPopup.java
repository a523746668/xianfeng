...
final class NavigationPopup extends ZLApplication.PopupPanel {
	...
	private void createPanel(FBReader activity, RelativeLayout root) {
		...
		final TextView mark = (TextView) myWindow.findViewById(R.id.navigation_mark);
		...
		mark.setOnClickListener(v -> {
			...
			final Intent intent =
					new Intent(myActivity, BookmarksActivity.class);
		...
		pre_character.setOnClickListener(v -> myActivity.turnChapter(false));

		next_character.setOnClickListener(v -> myActivity.turnChapter(true));
